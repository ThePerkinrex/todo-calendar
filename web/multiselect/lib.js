...
class MultiSelect {
	constructor(element, options = {}) {
		...
		const classList = this.selectElement.classList;
			...
			optionsHTML += `
                <div class="multi-select-option${
					this.selectedValues.includes(this.data[i].value)
						...
						: ""
				}" data-value="${this.data[i].value}" data-idx="${i}">
                    <span class="multi-select-option-radio"></span>
                    <span class="multi-select-option-text ${
						this.options.data[i].innerClasses != undefined
							? this.options.data[i].innerClasses.join(" ")
							: ""
					}" ${
				this.options.data[i].innerAttributes != undefined
					? Object.entries(this.options.data[i].innerAttributes)
							.map(([key, val]) => `${key}="${val}"`)
							.join(" ")
					: ""
			}>${
	...
	_generateHeaderOption(option) {
		const i = parseInt(option.dataset.idx);
		const data = this.options.data[i];

		return `<span class="multi-select-header-option ${
			this.options.data[i].outerClasses != undefined
				? this.options.data[i].outerClasses.join(" ")
				: ""
		}" data-value="${option.dataset.value}" data-idx="${i}" ${
			this.options.data[i].outerAttributes != undefined
				? Object.entries(this.options.data[i].outerAttributes)
						.map(([key, val]) => `${key}="${val}"`)
						.join(" ")
				: ""
		}>${
